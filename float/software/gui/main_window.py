...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.central_widget.setStyleSheet(f"background: {PALETTE['bg']};")
        ...
        # Set the layout ONCE here
        self._selector_layout = QHBoxLayout(self._selector_placeholder)
        self._selector_layout.setContentsMargins(8, 4, 8, 4)
    ...
    def _setup_column_selector(self, columns: list[str]):
        ...
        if self._col_selector:
            self._selector_layout.removeWidget(self._col_selector)
            self._col_selector.deleteLater()
            self._col_selector = None

        # Create new selector
        self._col_selector = ColumnSelector(columns, self._selector_placeholder)
        self._selector_layout.addWidget(self._col_selector)
        self._col_selector.selection_changed.connect(self._on_selection_changed)
