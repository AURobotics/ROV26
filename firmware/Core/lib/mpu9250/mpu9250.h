...
// See also MPU-9250 Register Map and Descriptions, Revision 4.0, RM-MPU-9250A-00, Rev. 1.4,
// 9/9/2013 for registers not listed in above document; the MPU9250 and MPU9150 are virtually
...
#define AK8963_CNTL                                                                                \
    0x0A // Power down (0000), single-measurement (0001), self-test (1000) and Fuse ROM (1111) modes
         // on bits 3:0
...
#define MOT_DUR                                                                                    \
    0x20 // Duration counter threshold for motion interrupt generation, 1 kHz rate, LSB = 1 ms
#define ZMOT_THR 0x21 // Zero-motion detection threshold bits [7:0]
#define ZRMOT_DUR                                                                                  \
    0x22 // Duration counter threshold for zero motion interrupt generation, 16 Hz rate, LSB = 64 ms
...
// uint8_t Mscale = MFS_16BITS; // MFS_14BITS or MFS_16BITS, 14-bit or 16-bit magnetometer
// resolution uint8_t Mmode = 0x06;        // Either 8 Hz 0x02) or 100 Hz (0x06) magnetometer data
// ODR float aRes, gRes, mRes;      // scale resolutions per LSB for the sensors
...
class MPU9250 {
    ...
    float magCalibration[3] = {0, 0, 0}, magbias[3] = {0, 0, 0},
          magscale[3] = {1.0f, 1.0f, 1.0f}; // Factory mag calibration and mag bias
    ...
    float GyroMeasError =
        M_PI * (5.0f / 180.0f); // gyroscope measurement error in rads/s (start at 60
    // deg/s), then reduce after ~10 s to 3
    // float beta = sqrt(3.0f / 4.0f) * GyroMeasError; // compute beta
    float beta = 0.4;
    float GyroMeasDrift =
        M_PI * (1.0f / 180.0f); // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
    float zeta =
    ...
    int lastUpdate = 0, firstUpdate = 0,
        Now = 0; // used to calculate integration interval // used to
...
public:
    ...
    void MahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz,
                                float mx, float my, float mz);
    void getEulerAngles(vec_3& angles);
    void getBodyRates(vec_3& rates);
    void calibrateMag(uint16_t num_samples);
